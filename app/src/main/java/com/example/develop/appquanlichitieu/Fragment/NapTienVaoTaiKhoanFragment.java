...
public class NapTienVaoTaiKhoanFragment extends Fragment {
    ...
    private void btnAddNew() {
        ...
        taiKhoan=new TaiKhoan(
                ...
                Float.valueOf(edtSoTienTK.getText().toString())
        );
        if(edtNameTK.getText().toString().equals(""))
        {
            err=false;
            edtNameTK.setError("yêu cầu thông tin");
        }
        else if(edtSoTienTK.getText().toString().equals(""))
        {
            err=false;
            edtSoTienTK.setError("Yêu cầu thông tin");
        }
        if(err==true)
        {
